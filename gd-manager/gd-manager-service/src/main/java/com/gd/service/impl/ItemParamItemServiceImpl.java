package com.gd.service.impl;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.gd.common.util.JsonUtils;
import com.gd.mapper.GdItemParamItemMapper;
import com.gd.pojo.GdItemParamItem;
import com.gd.pojo.GdItemParamItemExample;
import com.gd.pojo.GdItemParamItemExample.Criteria;
import com.gd.service.ItemParamItemService;
import com.sun.xml.internal.xsom.impl.scd.Iterators.Map;

@Service
public class ItemParamItemServiceImpl implements ItemParamItemService {
	
	@Autowired
	private GdItemParamItemMapper itemParamItemMapper;
	
	//通过商品id获得商品规格参数
	@Override
	public String getItemParamByItemId(Long itemId) {
		//根据商品id查询规格参数
		GdItemParamItemExample example = new GdItemParamItemExample();
		Criteria criteria = example.createCriteria();
		criteria.andItemIdEqualTo(itemId);
		//执行查询
		List<GdItemParamItem> list = itemParamItemMapper.selectByExampleWithBLOBs(example);
		if(list == null || list.size() == 0){
			return "";
		}
		//取规格参数信息
		GdItemParamItem itemParamItem = list.get(0);
		String paramData = itemParamItem.getParamData();
		//生成html
		//把规格参数json数据转换为java对象
		List<Map> jsonList = JsonUtils.jsonToList(paramData, Map.class);
		StringBuffer sb = new StringBuffer();
		sb.append("<table cellpadding=\"0\" cellspacing=\"1\" width=\"100%\" border=\"0\" class=\"Ptable\">\n");
		sb.append("    <tbody>\n");
		for(Map m1:jsonList) {
			sb.append("        <tr>\n");
			//sb.append("            <th class=\"tdTitle\" colspan=\"2\">"+m1.get("group")+"</th>\n");
			sb.append("        </tr>\n");
			List<Map> list2 = null;
			//List<Map> list2 = (List<Map>)  m1).get("params");
			for(Map m2:list2) {
				sb.append("        <tr>\n");
				//sb.append("            <td class=\"tdTitle\">"+m2.get("k")+"</td>\n");
				//sb.append("            <td>"+m2.get("v")+"</td>\n");
				sb.append("        </tr>\n");
			}
		}
		sb.append("    </tbody>\n");
		sb.append("</table>");
		return sb.toString();
	}
}
