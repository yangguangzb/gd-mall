...
import com.gd.search.service.SearchService;
...
public class SearchController {
	...
	@ResponseBody
	public GdResult search(@RequestParam("q")String queryString,
			@RequestParam(defaultValue="1")Integer page,
			@RequestParam(defaultValue="60")Integer rows){
		
		//查询条件不能为空
		if(StringUtils.isBlank(queryString)){
			return GdResult.build(400, "查询条件不能为空");
		}
		...
		return GdResult.ok(searchResult);
	}
